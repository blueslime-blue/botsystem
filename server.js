...
const youtube = new YouTube("AIzaSyApFg4t8RUKVHjeXl3VUFJMVT-V1WuaiHM"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyApFg4t8RUKVHjeXl3VUFJMVT-V1WuaiHM"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@514039736113954826>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "ولف تعال") {
    message.channel.send("**ايش في يا سيدي**");
    message.channel.sendFile("");
  }
});
client.on("ready", () => {
...
client.on("message", async message => {
  ...
  if (args[0].toLowerCase() == `${prefix}credits`) {
    ...
    if (!args[2]) {
      message.channel.send(
        `**${mention.username}, your anime_coins balance is \`$${credits[mention.id].credits}\`**`
      );
